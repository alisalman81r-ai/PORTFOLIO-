    ...
    >
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <GlowOrb
          tone="cool"
          motion="drift-slow"
          className="bottom-[5%] right-[10%] size-[45vw] max-w-[520px]"
        />
      </div>
      ...
      >
        ...
        <motion.div variants={item} className="relative grid place-items-center">
          ...
          */}
          <p aria-hidden="true" className={cn('relative heading-xl text-gradient tabular-nums')}>
            404
          </p>
        </motion.div>
        ...
        <motion.nav variants={item} aria-label="Site sections" className="mt-12">
          ...
          <ul className="mt-4 flex flex-wrap items-center justify-center gap-2">
            {FOOTER_LINKS.map((link) => (
              <li key={link.id}>
                {/* Route to `/` carrying the hash. The section lives on the home
                    page, so linking to the bare anchor from a 404 would target
                    an element that is not in this document. */}
                <Link
                  to={`/${link.href}`}
