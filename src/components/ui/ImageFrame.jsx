    ...
    >
      {src ? (
        <img
          src={src}
          alt={alt}
          // `lazy` is wrong here — the portrait is near the top of the page and
          // deferring it delays the largest element. `async` decoding keeps the
    ...
    >
      <div className="flex flex-col items-center gap-4 px-6 text-center">
        <span className="glass grid size-16 place-items-center rounded-full">
          <Icon name="user" className="size-7 text-faint" />
        </span>

        <span className="font-mono text-xs text-faint">
          Add your portrait
          <br />
          <span className="text-faint/70">PERSONAL.avatar</span>
        </span>
      </div>
    </div>
  )
